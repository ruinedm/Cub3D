...
{
	t_collision_info	collision;

	while (ray->next_ve_x >= 0 && ray->next_ve_x < cube->width
		&& ray->next_ve_y >= 0 && ray->next_ve_y <= cube->height)
	{
		collision = collides_with_wall_with_type(cube, ray->next_ve_x
				- ray->decrement, ray->next_ve_y);
		if (collision.collision_type)
		{
			*wall_hit_x = ray->next_ve_x;
			*wall_hit_y = ray->next_ve_y;
			ray->collision_type = collision.collision_type;
			if (collision.collision_type == WALL)
				return (true);
			if (collision.collision_type == DOOR)
			{
				collision.current_door->is_in_fov = true;
				if (!collision.current_door->is_open)
					return (true);
			}
		}
		...
		, t_ve_ray *ve_ray, t_ray *ray)
{
	...
	if (ve_ray->collision_type == DOOR)
		ren_info.texture = cube->textures.d_texture;
	else
	{
